import "../../Admin/Events/UpsertEvent.css"
...
import { createEvent, setEventSuccessFalse } from "../../../../Actions/EventAction"
...
export default function CreateEvent() {
    ...
    const formSubmitHandler = (e) => {
        ...
        if (!formdata.conductedClubs || formdata.conductedClubs.length <= 0) {
            ...
            dispatch(createEvent(fd, formdata.createdClub))
        }
    }
    useEffect(() => {
    ...
    useEffect(() => {
        if (clubs && user) {
            setUserClubs(clubs.filter((club) => {
    ...
    if (loading) { return <CoordinatorSetup current={"events"} option={"create"} element={<Loading />} /> }
    return (
        <CoordinatorSetup current={"events"} option={"create"} element={
            <section>
                    ...
                    <section>
                        ...
                        <div className="Upsert-event-search-clubs">
                            ...
                            <div>
                                ...
                                >
                                    {userClubs && userClubs.map((option) => (
