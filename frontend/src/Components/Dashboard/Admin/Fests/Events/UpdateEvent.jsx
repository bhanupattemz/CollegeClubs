...
import { updateFestEvent, setFestEventFalse, getFestEventDetails } from "../../../../../Actions/festActions"
import Loading from "../../../../Loaders/Loading"
import { useNavigate, useParams } from "react-router";
import axios from "axios"
import { BACKENDURL } from "../../../../Functionalities/functionalites"
import { CiBookmarkRemove } from "react-icons/ci";
import { FaMapLocationDot, FaS } from "react-icons/fa6";
...
export default function CreateEvent() {
    ...
    const formSubmitHandler = (e) => {
        ...
        if (!formdata.conductedClubs || formdata.conductedClubs.length <= 0) {
            ...
            if (formdata.fest) {
                fd.append('fest', formdata.fest)
            }
            dispatch(updateFestEvent(fd, _id))
        }
    }
    useEffect(() => {
    ...
    useEffect(() => {
        if (singleFestEvent) {
            setFormData({
                name: singleFestEvent.name,
                description: singleFestEvent.description,
                venueName: singleFestEvent.venue.venueName,
                landMark: singleFestEvent.venue.landMark,
                starting: dayjs(singleFestEvent.timings.starting),
                ending: dayjs(singleFestEvent.timings.ending),
                RegStarting: dayjs(singleFestEvent.registration.starting),
                RegEnding: dayjs(singleFestEvent.registration.ending),
                image: singleFestEvent.image,
                conductedClubs: singleFestEvent.conductedClub.map((item) => item._id),
                createdClub: singleFestEvent.createdClub,
                subheading: singleFestEvent.subheading,
                amount: singleFestEvent.amount,
                prizes: singleFestEvent.prizes,
                pdf: singleFestEvent.pdf
            })
            if (dayjs(singleFestEvent.timings.ending) < Date.now()) {
                setWantWinners(true)
            } else {
                setWantWinners(false)
            }
            setImgPrev(singleFestEvent.image.url)
            setConductedClubs(singleFestEvent.conductedClub)
        }
    }, [singleFestEvent])
            ...
            <section className="Upsert-fest-event-main-section">
                <form onSubmit={formSubmitHandler}>
                    ...
                    <section>
                        ...
                        {wantWinner && <Button onClick={() => navigate(`/admin/fests/events/update/winner/${_id}`)}>Update Winners</Button>}
                    </section>
