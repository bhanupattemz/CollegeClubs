...
export default function CreateClub() {
    ...
    useEffect(() => {
        ...
        if (singleClub) {
            setFormData({
                ...
                coordinators: singleClub.coordinators ? singleClub.coordinators.map((item) => item.details._id) : []
            })
            setRegistration(singleClub.registrationTiming)
            setBannerPrev(singleClub.bannerImage.url)
            setLogoPrev(singleClub.logo.url)
            setCordinators(singleClub.coordinators.map((item) => item.details))
        }
    }, [singleClub])
    useEffect(() => {
        if (_id) {
    ...
    if (clubLoading) {
        return <AdminSetUp current={"clubs"} option={"update"} element={<Loading />} />
    }

    return (
        <AdminSetUp current={"clubs"} option={"update"} element={
            <Fragment>
